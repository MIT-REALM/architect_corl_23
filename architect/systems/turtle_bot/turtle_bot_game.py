...
class Game(eqx.Module):
    ...
    def __call__(
        ...
        """
        e_sigma = jax.exp(sigma)
        T = self.duration
        exogenous_env = Environment_state(target_pos, e_sigma, x_inits)
        # initialize x_hists
        ...
        cost = [loss(e_sigma[n], get_squared_distance(trajectory, (jnp.zeros_like(trajectory[:,:,:2])+ target_pos[n]))) for n in range(self.n_targets)]
