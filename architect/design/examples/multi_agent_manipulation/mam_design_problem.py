...
def make_mam_design_problem_friction_only(
    layer_widths: Tuple[int], dt: float, prng_key: PRNGKeyArray
) -> DesignProblem:
    """Make an instance of the multi-agent manipulation design problem.
    Uses two turtlebots. Only varies friction (not desired box pose or mass)
    ...
    mu_turtle_ground_range = jnp.array([0.6, 0.8])
    mu_box_ground_range = jnp.array([0.4, 0.6])
    box_mass_range = jnp.array([1.0, 1.0])
    desired_box_pose_range = jnp.array(
        [
            [0.3, 0.3],
            [0.3, 0.3],
    ...
    turtlebot_displacement_covariance = 1e-6 * (0.1 ** 2) * jnp.eye(3)
    ep = MAMExogenousParameters(
        mu_turtle_ground_range,
        mu_box_ground_range,
        mu_box_turtle_range,
