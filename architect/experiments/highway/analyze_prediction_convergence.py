...
lr = 1e-2
lr = f"{lr:.1e}"
SUMMARY_PATH = f"results/highway/predict/convergence_summary_{lr}.json"
...
DATA_SOURCES = {
    ...
    "rmh": {
        "path_prefix": f"results/highway/predict/noise_5.0e-01/L_1.0e+01/25_samples/5_chains/0_quench/dp_{lr}/ep_{lr}/rmh",
        "display_name": "ROCUS",
    },
    "gd": {
        "path_prefix": f"results/highway/predict/noise_5.0e-01/L_1.0e+00/25_samples/5_chains/0_quench/dp_{lr}/ep_{lr}/gd",
        "display_name": "ML",
    },
    "reinforce": {
        "path_prefix": f"results/highway/predict/noise_5.0e-01/L_1.0e+00/25_samples/5_chains/0_quench/dp_{lr}/ep_{lr}/reinforce_l2c",
        "display_name": "L2C",
    },
}
        ...
        for seed in SEEDS:
            with open(DATA_SOURCES[alg]["path_prefix"] + f"_{seed}" + ".json") as f:
                ...
                new_data = {
                    ...
                    "failure_level": 3.6,  # data["failure_level"],  # more accurate
                ...
                summary_data[alg].append(
                    {
                        ...
                        "failure_level": 3.6,  # result["failure_level"], # more accurate
                    }
                )
        ...
        with open(SUMMARY_PATH, "rb") as f:
            ...
            for alg in summary_data:
                for result in summary_data[alg]:
                    ...
                    result["failure_level"] = 3.6  # measured from plots
